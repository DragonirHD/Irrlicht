...
export class ThemeToggleComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    if (localTheme) {
      this.isDarkMode = localTheme == this.localThemeDarkName;
    } else {
      this.isDarkMode = window.matchMedia('(prefers-color-scheme: dark)').matches;
    }

    if (this.isDarkMode) {
      document.documentElement.classList.add(this.localThemeDarkName);
    }
  ...
  ngAfterViewInit() {
    if (this.themeToggleElement) {
      this.themeToggleElement.nativeElement.querySelector('.mdc-switch__icon--on').firstChild.setAttribute('d', this.moon);
      this.themeToggleElement.nativeElement.querySelector('.mdc-switch__icon--off').firstChild.setAttribute('d', this.sun);
    }
  }
  ...
  private setTheme() {
    if (this.isDarkMode) {
      document.documentElement.classList.add(this.localThemeDarkName);
      document.documentElement.classList.remove(this.localThemeLightName);
      localStorage.setItem(this.localThemeKey, this.localThemeDarkName);
    } else {
      document.documentElement.classList.add(this.localThemeLightName);
      document.documentElement.classList.remove(this.localThemeDarkName);
      localStorage.setItem(this.localThemeKey, this.localThemeLightName);
    }
  }
}
