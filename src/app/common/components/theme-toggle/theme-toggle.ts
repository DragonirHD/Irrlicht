...
export class ThemeToggleComponent implements OnInit, AfterViewInit {
  ...
  private setTheme() {
    if (this.isDarkMode) {
      document.body.classList.add(this.localThemeDarkName);
      document.body.classList.remove(this.localThemeLightName);
      localStorage.setItem(this.localThemeKey, this.localThemeDarkName);
    } else {
      document.body.classList.add(this.localThemeLightName);
      document.body.classList.remove(this.localThemeDarkName);
